...
class RL_Agent:
    def __init__(self, env, mem_size, gamma, epsilon, epsilon_decay, epsilon_end, batch):
        ...
        self.writer = SummaryWriter('logs/SMBDQN') #Logging
    ...
    def train(self, train_episodes):
        ...
        for episode_count in range(train_episodes):
            ...
            if(episode_count % 10 == 0): # Update user
                ...
                self.save_model(self.best,"bestDQN.pt") #save current model
        ...
        self.save_model(self.best,"bestDQN.pt") #actual save    
    ...
    def unit_test(self):
        ...
        for step in range (10000000):
            if done:
                self.env.reset()
            s, r, done = self.env.step(random.randrange(7))
            print(r)
            time.sleep(0.125)
        self.env.close()
