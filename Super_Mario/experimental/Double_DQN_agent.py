...
class RL_Agent:
    ...
    # Define base directories for models and logs
        base_model_dir = os.path.join(parent_dir, "TrainModelResults")
        base_log_dir = os.path.join(parent_dir, "TrainLogs")
    ...
    def pick_alt_action(self, state):
        ...
        with torch.no_grad():
            ...
            progress = min(self.episode_count / self.death_weights_ep_transition, 1.0)  # 2500 episodes to reach full exploration
    ...
    # Build the log message with line separators
        separator = "-" * 30
        log_message = (
            f"{separator}\n"
